...
@section('konten')
    <main class="h-full min-h-screen bg-cover bg-center bg-no-repeat p-10 transition-all duration-300 ease-in-out lg:pl-88"
        ...
        @foreach ($tahun as $thn)
            <div
                class=" mt-10 flex flex-col items-start w-96 h-auto p-6 rounded-xl shadow-lg bg-green-dark border-2 border-white text-white hover:shadow-2xl transition-shadow duration-300 lg:mr-10">
                <h2 class="text-2xl font-bold mb-4">Rekap {{ $thn }}</h2>
                <a href="{{ url("admin/data-kecamatan/rekap-kecamatan/$id/tahun/$thn") }}"
