    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
                    ...
                    formulir.addEventListener("submit", async (e) => {
                        ...
                        const masukan = formulir.querySelectorAll(
                        ...
                        if (window.daftar_pangan.length === 0) {
                            alert(
                                'Harap tambahkan setidaknya satu item pangan ke dalam tabel sebelum mengirimkan formulir!');
                            return;
                        }
...
               }
                            try {
                                ...
                                if (response_text.trim().startsWith('<!DOCTYPE html>'))
                                throw new Error(
                                        'Server mengembalikan halaman HTML, mungkin terjadi kesalahan pada server.'
                                        );
                                ...
                                if (parse.redirect) {
                                    ...
                                    tombol_kirim.innerHTML =
                                        `<i class="fa-solid fa-paper-plane mr-4"></i> Kirim`;
                                    tombol_kirim.disabled = false;
                                }
                            } catch (error) {
                                console.error('[ERROR] Terjadi kesalahan saat proses mengirim: ',
                                    error);
                                tombol_kirim.innerHTML =
                                    `<i class="fa-solid fa-paper-plane mr-4"></i> Kirim`;
                                tombol_kirim.disabled = false;
